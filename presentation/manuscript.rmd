...
were to be resampled. To investigate whether the sampling had an impact on
...
extent of the arising artifacts. To this end, we have tested `r
length(lfc_thresholds)` different threshold values for the log~2~ fold
...
DEGs and of observed artifacts (Fig. 3, A and B). However, even for log~2~
threshold of 3 (DEGs defined by 8-fold change and FDR < `r pval_thr`) the
...
enriched. Thus, setting a more conservative threshold cannot protect from
the arising artifacts.

Contrary to our expectations, the number of artifacts rose with sample size
(Fig. 3, C and D). For total sample size of 160 (40 samples per
