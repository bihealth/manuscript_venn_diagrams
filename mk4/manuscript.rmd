---
title: "Venn diagrams may indicate erroneous statistical approaches leading to artifacts in transcriptomic analyses"
...
subsets is a fallacy.  A statistically correct approach involves testing for interaction.
Moreover, since genes which show a significant change in one condition but
not in another are likely to be false negatives in the latter case, these
genes considered to be specific for one condition are likely characteristic
for both of the compared groups, and enriched in functions related to the
research hypothesis. Thus, the combination of incorrect statistical
analysis and gene set enrichment analysis may result in particularly
...
padj_thr <- 0.05
lfc_thr  <- 1
n_stratum <- 20 # 2 strata per condition 
```
...
and which are regulated only in one of them.  The VD serves as a basis for
the statement that the regulation of certain genes is "specific" for one
strain or another. A gene is, in this context, considered "specific" or
"unique" for a given comparison when it shows a statistically significant
difference in that comparison only.  We argue that not only is this
...
data set only once. To investigate whether the sampling had an impact on
...
change threshold
or different sample size might have an impact on the extent of the arising
artifacts. To this end, we have tested `r length(lfc_thresholds)` different
threshold values for the log~2~ fold changes and three sample sizes: 20, 40 or 80 samples
per group (corresponding to 10, 20 or 40 samples per group/treatment
combination). 

**Fig. 3.Influence of log~2~ fold change treshold (A, C) and the sample
...
```{r montecarlo,cache=TRUE,fig.width=5.5,fig.height=5.5}
...
were drawn (Fig. 4), comparing the 
...
**Fig. 4. Area-proportional Venn diagram showing overlaps in DEGs between
...
In an informal survey of articles from three journals (Tab. 2) we found
that of the 
`r tab2$Analysed[nrow(tab2)]` analysed articles
 which used the term "venn diagram" and
...
Statistical analysis, as shown above, involves a test for interaction which
...
While the use of VDs in combination with the incorrect gene set
...
applications. VDs appear to be symptomatic of the assumption that if a gene
is significantly regulated in one comparison, but does not show a
significant difference in another one, then this gene is "specific" to this
first comparison. However, VDs may also lead one to this reasoning; for
example, the VD on Fig. 1A practically begs the question: what do these 
`r sum(res.merged$DEG.g1 & !res.merged$DEG.g2)` 
genes, which are significant in G1 only, have in common?
