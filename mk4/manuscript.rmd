---
title: "Venn diagrams as indicators of erroneous statistical reasoning in transcriptomics."
...
A common application of differential expression analysis is to
find genes that are differentially expressed upon treatment in only one
...
Here we show that this procedure is statistically incorrect and that the interaction
between treatment and group should be tested instead. Moreover, we show
that gene set enrichment analysis applied to such incorrectly defined genes
group-specific genes only may result in misleading artifacts. Genes which
show significant difference upon treatment in one, but not another group
are enriched in gene sets associated with the treatment in general.  Thus,
the results appear related to the problem at hand, but do not reflect the
group-specific effect of a treatment. A literature search revealed that
more than a quarter of papers which used a Venn diagram to illustrate the
results of separate differential analysis  have also applied this incorrect
reasoning.
...
Experimental designs for transcriptomic analyses commonly include more than one factor. Often, the
...
not regulated in another group of patients. In other words, we ask whether the
...
separately in both groups. Next, a difference between groups is inferred if the difference
...
between the group and the treatment. However, often the differences between
treatment and control are tested in the two groups separately, and the
results are then compared. This can be visualized using a Venn diagram (VD,
Fig. 1) showing the overall number of DEGs significant in both comparisons
(the intersection in the VD) or significant in only one comparison (the
remaining two fields on a VD).  The genes which are significant in only one
comparison may then be incorrectly defined as specifically to the corresponding
group. Following this, gene set enrichment analysis may be applied to
these genes in an attempt to test which pathways are specific to one, but
not the other group.
...
uncommon. In fact, while VDs are a useful visualisation tool also in
...
groups, and in 19% a gene set enrichment was performed.
